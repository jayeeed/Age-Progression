    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if self.dataset == 'CelebA':
                ...
                c_third = self.label2onehot(label_third, self.c_dim)

            x_real = x_real.to(self.device)           #input images
            x_real = torch.squeeze(x_real)
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake_B = self.G(x_real, c_trg) #for identity
                x_fake = torch.tanh(x_real + x_fake_B)  #Changed
                out_src, out_cls = self.D(x_fake)
                g_loss_fake = - torch.mean(out_src)
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset)

                # Original-to-original domain for identity 
                x_fake_B_id = self.G(x_real, c_org)
                x_fake_id = torch.tanh(x_real + x_fake_B_id)
                ...
                #identity loss
                g_loss_id = torch.mean(torch.abs(x_real - torch.tanh(x_fake_B_id + x_real)))

                # Target-to-original domain.
                x_fake_B_reconst = self.G(x_fake, c_org) #For identity
                x_reconst = torch.tanh(x_fake + x_fake_B_reconst)
                g_loss_rec = torch.mean(torch.abs(x_real - x_reconst))

                # Original-to-original domain.
                x_fake_B_reconst_id = self.G(x_fake_id, c_org)
                x_reconst_id = torch.tanh(x_fake_id + x_fake_B_reconst_id)
                g_loss_rec_id = torch.mean(torch.abs(x_real - x_reconst_id))

                # ================================================================================#
                #                      Triple consistency loss & Pixel Loss.                          #
                # ================================================================================#
                ...
                loss_pixel = mae_loss(x_fake, x_real)


                # Backward and optimize.
                g_loss_new = g_loss_fake_id + self.lambda_rec * g_loss_rec_id + self.lambda_cls * g_loss_cls_id + self.lambda_id * g_loss_id
                g_loss = g_loss_fake + self.lambda_rec * (g_loss_rec + g_loss_triple) + self.lambda_cls * g_loss_cls + g_loss_triple + loss_pixel + g_loss_new
                ...
                loss['G/pixel_loss'] = loss_pixel.item()
